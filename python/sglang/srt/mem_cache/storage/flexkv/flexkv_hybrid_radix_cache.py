...
class FlexKVHybridRadixCache(BasePrefixCache):
    ...
    def match_prefix(self, params: MatchPrefixParams) -> MatchResult:
        ...
        if self.flexkv_connector.has_mamba:
            mamba_hit = self.flexkv_connector.lookup_mamba_state(
                token_ids=token_ids, rid=params.req.rid,
            )
            if mamba_hit > 0:
                logger.debug(
                    "[FlexKV-Mamba] match_prefix mamba hit=%d rid=%s",
                    mamba_hit, params.req.rid,
                )
    ...
    def cache_finished_req(self, req: Req, is_insert: bool = True, **kwargs) -> None:
        ...
        # radix_branch: detect branch point before insertion
        branch_token_ids = None
        if is_insert and self.flexkv_connector.has_mamba and len(token_ids) > 1:
            try:
                pre_key = RadixKey(
                    array("q", list(token_ids)),
                    req.extra_key,
                    is_bigram=bool(getattr(self._inner_cache, "is_eagle", False)),
                )
                pre_match = self._inner_cache.match_prefix(MatchPrefixParams(key=pre_key))
                pre_hit = pre_match.device_indices.numel() if pre_match.device_indices is not None else 0
                if 0 < pre_hit < len(token_ids):
                    branch_token_ids = list(token_ids[:pre_hit])
            except Exception:
                pass
        ...
        if self.flexkv_connector.has_mamba:
            ...
            if mamba_pool_idx is not None and mamba_pool_idx >= 0:
                try:
                    ...
                    logger.debug("[FlexKV-Mamba] store in _store_prefix: %s", exc)
